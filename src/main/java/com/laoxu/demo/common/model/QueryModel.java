...
public class QueryModel<T extends QueryModel> implements Serializable {
    ...
    @JsonIgnore
    private String deleteTime = BaseObject.VAL_NOT_DELETED; // 删除时间，为空串时表示未删除,格式yyyyMMddhhmmssSSS
    @JsonIgnore
    private String[] properties; // 限制返回列名称列表

    /**
     * @Description 设置页码和每页大小，不符合的使用默认值
     * @author linb 2017年6月1日 下午12:04:03
    ...
    public void startPage() {
        PageHelper.startPage(getPageNum(), getPageSize(), getCountSql(), getReasonable(),
                getPageSizeZero());
    }
}
