...
import com.laoxu.demo.common.model.QueryModel;
...
public abstract class DbUtils {
    ...
    public static Example.Criteria getCriteria(Example example) {
        return example.createCriteria().andEqualTo(BaseObject.PROP_DELETE_TIME,
                BaseObject.VAL_NOT_DELETED);
    }
    ...
    public static Example.Builder newExampleBuilder(Class<?> clazz) {
        return Example.builder(clazz).andWhere(
                Sqls.custom().andEqualTo(BaseObject.PROP_DELETE_TIME, BaseObject.VAL_NOT_DELETED));
    }
    ...
    public static Example.Builder newExampleBuilder(Class<?> clazz, QueryModel queryModel) {
        ...
        builder.andWhere(
                Sqls.custom().andEqualTo(BaseObject.PROP_DELETE_TIME, queryModel.getDeleteTime()));
    ...
    public static <T> Optional<T> getOne(PageInfo<T> pageInfo) {
        return CollectionUtils.isEmpty(pageInfo.getList()) ? Optional.empty()
                : Optional.ofNullable(pageInfo.getList().get(0));
    }
     ...
     * @param collection
     */
    public static void setCollectionProp(Example.Builder builder, String property,
        ...
        if (1 == collection.size()) {
            ...
            builder.andWhere(Sqls.custom().andEqualTo(BaseObject.PROP_DELETE_TIME, "ywwuyi"));
        } else {
    ...
    /**
     * 查看
     * {@link DbUtils#setEqualToProp(Example.Builder, String, Object, boolean)}
     * 不允许blank str
     * @param builder
     * @param property
     * @param value
